 ...
 * @return array
 */
function bastille_body_classes( $classes ) {
 ...
 */
function bastille_pingback_header() {
...
if(!function_exists('bastille_custom_breadcrumbs')) {
   ...
   */
    ...
    function bastille_modify_post_thumbnail_html($html, $post_id, $post_thumbnail_id, $size, $attr) {
        if( '' == $html ) {
            $attr['class'] = 'default';
            global $post, $posts;
            ob_start();
            ...
            bastille_get_first_image($post_id);
            $first_img = bastille_get_first_image($post_id);
            if ( !empty( $first_img ) ){
     ...
     *
     */
    add_action( 'init', 'bastille_register_nav_menus' );

    function bastille_register_nav_menus() {
        register_nav_menu( 'social', __( 'Social', 'bastille' ) );
    }
    ...
    function bastille_register_event_post_type() {
        // Set UI labels for Custom Post Type
        $labels = array(
            'name'                => _x( 'Event', 'Event', 'bastille'),
            'singular_name'       => _x( 'Event', 'Event', 'bastille' ),
            'menu_name'           => __( 'Events', 'bastille' ),
            'parent_item_colon'   => __( 'Parent', 'bastille' ),
            'all_items'           => __( 'All', 'bastille' ),
            'view_item'           => __( 'View', 'bastille' ),
            'add_new_item'        => __( 'Add New', 'bastille' ),
            'add_new'             => __( 'Add', 'bastille' ),
            'edit_item'           => __( 'Edit', 'bastille' ),
            'update_item'         => __( 'Update', 'bastille' ),
            'search_items'        => __( 'Search', 'bastille' ),
            'not_found'           => __( 'Not Found', 'bastille' ),
            'not_found_in_trash'  => __( 'Not found in Trash', 'bastille' ),
        );

        // Set other options for Custom Post Type

        $args = array(
            'label'               => __( 'Events', 'bastille' ),
            'description'         => __( 'Past and future events', 'bastille' ),
            'labels'              => $labels,
    ...
    function bastille_register_programme_post_type() {
        // Set UI labels for Custom Post Type
        $labels = array(
            'name'                => _x( 'Programme', 'Programme', 'bastille'),
            'singular_name'       => _x( 'Programme', 'Programme', 'bastille' ),
            'menu_name'           => __( 'Programmes', 'bastille' ),
            'parent_item_colon'   => __( 'Parent', 'bastille' ),
            'all_items'           => __( 'All', 'bastille' ),
            'view_item'           => __( 'View', 'bastille' ),
            'add_new_item'        => __( 'Add New', 'bastille' ),
        ...
        $args = array(
            'label'               => __( 'Programmes', 'bastille' ),
            'description'         => __( 'List of programmes', 'bastille' ),
            'labels'              => $labels,
            // Features this CPT supports in Post Editor
            'supports'            => array( 'title', 'editor', 'excerpt', 'author', 'thumbnail', 'comments', 'revisions', 'custom-fields', ),
            // You can associate this CPT with a taxonomy or custom taxonomy. 
    ...
    function bastille_custom_post_types() {
        // Registering your Custom Post Type
        bastille_register_programme_post_type();
        bastille_register_event_post_type();
    }

    add_action( 'init', 'bastille_custom_post_types', 0 );
